...
// import {errorResult, valueResult, mappedValueResult} from '../jsutil/domain/Result';
import {ResultModule, VowModule} from "lib/js-util/index";

let {valueVow, errorVow} = VowModule;
let {errorResult, valueResult, mappedValueResult} = ResultModule;
...
test('test flatMap error vow', async () => {
  const error = 'error';
  let vow = errorVow(error);
  let mappedVow = vow.flatMap(mappedValueResult(times(100)));
  mappedVow.then(result => {
