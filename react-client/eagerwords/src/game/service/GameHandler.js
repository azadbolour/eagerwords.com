...
import {stringify} from 'lib/js-util/index';
 import {resultVow} from "../../lib/js-util/domain/Vow";
...
// import {resolveVow} from "../../jsutil/domain/Vow";
// import {valueResult} from "../../jsutil/domain/Result";
import {ResultModule, VowModule} from "lib/js-util/index";

let {resolveVow} = VowModule;
...
export const mkGameHandler = function(maybeLoginEvidence) {
  ...
  let handler = {
    ...
    checkRunning: function(game, actionName) {
      let ok = game.running();
      let result = ok ? valueResult(game) : mkUiErrorResult(errorTags.terminatedGame);
      return resultVow(result);
    },
    ...
    commitPlayAndGetMachinePlay: function(game) {
      // console.log(`commitPlayAndGetMachinePlay: ${stringify(game)}`);
    ...
    /**
     * Get all the information about a game.
     */
    ...
    commitPlayInternal: function(game) {
      let playPiecesResult = game.getCompletedPlayPieces();
