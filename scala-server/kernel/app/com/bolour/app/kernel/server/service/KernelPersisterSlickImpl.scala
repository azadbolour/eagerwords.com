...
import com.bolour.util.{ISymmetricCrypt, SymmetricCrypt, SymmetricCryptJava}
...
/**
  ...
  */
class KernelPersisterSlickImpl(val profile: JdbcProfile, db: Database, crypter: SymmetricCrypt) extends KernelPersister {
...
object KernelPersisterSlickImpl {

  val dbConfigPrefix = "service.db"
  def confPath(pathInDb: String) =  s"${dbConfigPrefix}.${pathInDb}"

  def apply(dbName: String, config: Config, secretService: SecretService): KernelPersisterSlickImpl = {
    ...
    new KernelPersisterSlickImpl(myProfile, myDb, crypter)
  }
}
