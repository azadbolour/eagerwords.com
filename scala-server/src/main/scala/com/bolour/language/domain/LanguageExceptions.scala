...
object LanguageExceptions {
  ...
  case class MissingDictionaryException(languageCode: String, path: String, cause: Throwable) extends LanguageException(cause) {
    override def getMessage: String = s"unable to read dictionary for language code: ${languageCode} from ${path}${causeMessage}"
  }
