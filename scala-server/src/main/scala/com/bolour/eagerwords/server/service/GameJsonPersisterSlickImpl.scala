...
// TODO. Add indexes.
// game by user_id for example.

// TODO. Optimization. Split the game table into 2: running and completed games.
// To speed up normal game playing. For now premature optimization.
// Use transaction (slick transactionally) to move completed game out of
// running and into completed table.
...
class GameJsonPersisterSlickImpl(val profile: JdbcProfile, db: Database) extends GameJsonPersister {
  ...
  class GameTable(tag: Tag) extends Table[GameRow](tag, gameTableName) {
    ...
    def userIdIndex = index("game_user_id_index", userId)
  }
  ...
  override def getGuestGames(): Future[List[String]] = {
    val query = for {
      gameRow <- gameRows.filter(_.userId.isEmpty)
    } yield gameRow;
  ...
  override def getUnfinishedUserGames(userId: ID): Future[List[String]] = {
    val query = for {
      gameRow <- gameRows if gameRow.userId === userId && gameRow.state === GameState.RUNNING.toString
    } yield gameRow;
    val q = query.result.map(seq => seq.toList.map(row => row.json))
    db.run(q)
  }
