...
trait GameService {
  ...
  def removeAllUserGameRelatedInfo(email: String): Future[Unit]
  ...
  /**
    ...
    * @param maybeEmail The user's email as an option. None implies user is a guest.
  ...
  /**
    * Harvest long-running games - considering them as abandoned.
    */
  def timeoutLongRunningGames(): Future[Unit]

  def timeoutLongRunningGames(timeout: Long): Future[Unit]
