...
import com.bolour.eagerwords.controller.GameDtoConverters._
import play.api.libs.json._

// Needed for implicits (Intellij does not realize it)!
...
object GameApiJsonSupport {
  ...
  implicit val illegalGuestOpErrorDtoWrites: OWrites[IllegalGuestOpErrorDto] = writes[IllegalGuestOpErrorDto]
  implicit val completedGameOpErrorDtoWrites: OWrites[CompletedGameOpErrorDto] = writes[CompletedGameOpErrorDto]
  ...
  def gameExceptionToUnprocessableEntity(ex: GameException): Result = {
    ex match {
      ...
      case ex: IllegalGuestOpException => unprocessableJson(toIllegalGuestOpErrorDto(ex))
      case ex: CompletedGameOpException => unprocessableJson(toCompletedGameOpErrorDto(ex))
    }
  }

  def languageExceptionToUnprocessableEntity(ex: LanguageException): Result = {
    ex match {
